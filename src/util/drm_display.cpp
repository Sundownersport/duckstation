...
static uint32_t find_crtc_for_connector(int card_fd, const drmModeRes* resources, const drmModeConnector* connector)
{
  ...
  {
    drmModeEncoder* encoder = drmModeGetEncoder(card_fd, connector->encoders[i]);
    if (encoder)
    {
...
bool DRMDisplay::TryOpeningCard(int card)
{
  ...
  const std::string card_path = fmt::format("/dev/dri/card{}", card);
  m_card_fd = open(card_path.c_str(), O_RDWR);
  if (m_card_fd < 0)
  {
    ERROR_LOG("open({}) failed: {} ({})", card_path, errno, strerror(errno));
    return false;
  }
  ...
  if (!resources)
  {
    ERROR_LOG("drmModeGetResources() failed: {} ({})", errno, strerror(errno));
    return false;
  }
  ...
  INFO_LOG("Using DRM card {} ({}x{})", card, m_mode->hdisplay, m_mode->vdisplay);
  return true;
}
...
std::optional<u32> DRMDisplay::AddBuffer(u32 width, u32 height, u32 format, u32 handle, u32 pitch, u32 offset)
{
  ...
  if (res != 0)
  {
    ERROR_LOG("drmModeAddFB2() failed: {}", res);
    return std::nullopt;
  }
...
void DRMDisplay::PresentBuffer(u32 fb_id, bool wait_for_vsync)
{
  if (!wait_for_vsync)
  {
    ...
    if (res != 0)
      ERROR_LOG("drmModeSetCrtc() failed: {}", res);
    return;
  }
  ...
  if (res != 0)
  {
    ERROR_LOG("drmModePageFlip() failed: {}", res);
    return;
  }
  ...
  while (waiting_for_flip)
  {
    ...
    int sres = select(m_card_fd + 1, &fds, nullptr, nullptr, nullptr);
    if (sres < 0)
    {
      ERROR_LOG("select() failed: {}", errno);
      break;
    }
    else if (sres == 0)
    {
